...
st.write(
    "Upload your CSV file to perform multi-stage local verification linked directly to your Blocklist Manager."
)
...
if uploaded_file is not None:
    ...
    email_col = st.selectbox(
        "Select the column containing email addresses:", columns
    )

    if st.button("🚀 Start Verification Process", type="primary"):
        with st.spinner(
            "Processing emails against Syntax, MX Records, and Blocklist..."
        ):
            valid_indices = []
            blocked_count = 0
            invalid_count = 0
            ...
            for idx, row in df.iterrows():
                ...
                status, reason = verify_single_email(
                    email_val, current_blocklist
                )
